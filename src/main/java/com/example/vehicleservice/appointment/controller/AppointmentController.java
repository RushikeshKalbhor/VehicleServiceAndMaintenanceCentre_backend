...
public class AppointmentController {
    ...
    @Parameter(name = "vehicleNumber", description = "This is the vehicle number", schema = @Schema(type = "string", minLength = 6, maxLength = 15), required = false)
    @Parameter(name = "pageNumber", description = "This is the page number", schema = @Schema(type = "integer", minimum = "1", maximum = "8388607"), required = false)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation", content = @Content(schema = @Schema(description = "Shows found/not found",
    ...
    public ResponseEntity<ResponseJson> getAdminAppointmentList(@RequestParam (required = false) @Size(min = 6, max = 15) String vehicleNumber, @RequestParam (required = false) @Min(1) @Max(8388607) Integer pageNumber) {
    ...
    @Operation(summary = "Get today's appointment", description = "This API is used to get today's appointment", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation", content = @Content(schema = @Schema(description = "show Found/Not found",
                    example = """
                            today.appointment.details.found : Appointment details found successfully,
                            today.appointment.details.not.found : Appointment details not found
                            """))) })
    @GlobalApiResponses
    @GetMapping("/today/appointment")
    public ResponseEntity<ResponseJson> getTodayAppointment() {
        return ResponseEntity.status(HttpStatus.OK).body(appointmentService.getTodayAppointment());
    }
}
